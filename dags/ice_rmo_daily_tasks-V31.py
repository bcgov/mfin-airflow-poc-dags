...
def email_completion():
    dYmdHMS = pendulum.now().substract(days=1).format("YYYY-MM-DD HH:mm:ss")
...
def email_notification():
    ...
    dYmdHMS = pendulum.now().substract(hours=7).format("YYYY-MM-DD HH:mm:ss")
    ...
    dYmdHMS = pendulum.today().substract(days=1).format("YYYY-MM-DD")
...
def etl_daily_load():
    ...
    def load_db_source(pSourceFile, pDBName):
        ...
        dYmd = pendulum.today().substract(days=1).format("YYYY-MM-DD")
        #dYmd = (dt.datetime.today() + timedelta(days = -1)).strftime('%Y%m%d')
